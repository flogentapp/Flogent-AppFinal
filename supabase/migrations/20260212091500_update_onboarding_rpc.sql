...
-- Update Onboarding RPC to support Service Role calls (with explicit user_id)
...
DECLARE
    ...
    -- NEW: explicitly passed user_id for service role calls
    v_explicit_user_id UUID := (payload->>'user_id')::uuid;
BEGIN
    -- 0. Set Bypass Flag for Triggers
    PERFORM set_config('app.is_onboarding', 'true', true);

    -- 1. Get current user ID (prefer explicit if provided, fallback to auth.uid)
    v_user_id := COALESCE(v_explicit_user_id, auth.uid());
    
    IF v_user_id IS NULL THEN
        RAISE EXCEPTION 'User ID is required (either via auth.uid() or payload.user_id)';
    END IF;
