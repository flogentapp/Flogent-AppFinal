...
import { AssignProjectLeaderModal } from './AssignProjectLeaderModal'
...
export function ProjectsClient({ projects, users, currentCompanyId }: ProjectsClientProps) {
    const [selectedProject, setSelectedProject] = useState<any>(null)
    const [assignLeaderProject, setAssignLeaderProject] = useState<any>(null)
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {projects.map((project) => (
                    ...
                    >
                        ...
                        {/* Make title clickable to open edit sheet? Or just use 'Edit Details' button */}
                        <button
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-50 flex items-center justify-between w-full">
                            <button
                                onClick={() => setAssignLeaderProject(project)}
            ...
            {assignLeaderProject && (
                <AssignProjectLeaderModal
                    isOpen={!!assignLeaderProject}
                    onClose={() => setAssignLeaderProject(null)}
                    projectId={assignLeaderProject.id}
                    projectName={assignLeaderProject.name}
                    users={users}
                />
            )}
        </div>
