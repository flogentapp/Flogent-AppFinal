...
import { updateTimeEntry } from '@/lib/actions/timesheets'
import { Loader2, AlertCircle } from 'lucide-react'
import { useRouter } from 'next/navigation'

export function EditEntryModal({ isOpen, onClose, projects, entry }: any) {
    ...
    useEffect(() => {
        if (entry) {
            setIsAdditional(entry.is_additional_work || false)
        }
    }, [entry])
                ...
                <form action={handleSubmit} className='space-y-4'>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <Input type='date' name='date' required defaultValue={entry.entry_date || entry.date} />
                        </div>
                        <div>
                            <label className='block text-xs font-bold text-gray-500 uppercase mb-1'>Hours</label>
                            <Input type='number' name='hours' step='any' required defaultValue={entry.hours} placeholder='e.g. 4.12' />
                        </div>
                    </div>

                    <div>
                        <label className='block text-xs font-bold text-gray-500 uppercase mb-1'>Project</label>
                        <select name='project_id' className='w-full border rounded-md p-2 bg-white text-sm h-10' required defaultValue={entry.project_id}>
                    ...
                    <div>
                        ...
                        <textarea name='description' className='w-full border rounded-md p-2 text-sm' rows={2} defaultValue={entry.description} placeholder='Standard work description...'></textarea>
                    </div>
                        ...
                        {isAdditional && (
                            <div className="animate-in fade-in slide-in-from-top-1 duration-200">
                                ...
                                <textarea
                                    ...
                                    defaultValue={entry.additional_work_description}
                    ...
                    <Button type='submit' className='w-full' disabled={loading}>
                        {loading ? <Loader2 className='animate-spin w-4 h-4' /> : 'Update Entry'}
                    </Button>
                </form>
            </DialogContent>
        </Dialog>
