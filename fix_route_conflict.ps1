Write-Host "--- MOVING PROJECT PAGE TO ADMIN ROUTE ---" -ForegroundColor Cyan

# 1. DELETE the conflicting folder (app/project)
if (Test-Path "app\project") {
    Remove-Item -Recurse -Force "app\project"
    Write-Host "✅ Removed conflicting folder: app\project" -ForegroundColor Yellow
}

# 2. CREATE the correct folder: app/admin/projects/[id]
# We use separate params to handle the brackets safely
if (-not (Test-Path -LiteralPath "app\admin\projects\[id]")) {
    New-Item -ItemType Directory -Path "app\admin\projects" -Name "[id]" -Force | Out-Null
    Write-Host "✅ Created folder: app\admin\projects\[id]" -ForegroundColor Green
}
...
$filePath = "app\admin\projects\[id]\page.tsx"
$pageContent | Out-File -LiteralPath $filePath -Encoding UTF8
Write-Host "✅ Page moved to: $filePath" -ForegroundColor Green