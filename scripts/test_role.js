...
async function findEnums() {
    ...
    const { error: testError } = await supabase.from('user_role_assignments').insert({
        user_id: testUserId,
    ...
    if (testError) {
        console.log('Insert "DepartmentHead" failed:', testError.message);
    } else {
        console.log('Insert "DepartmentHead" succeeded!');
    }
}
